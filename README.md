...
export namespace Job {
  export type Data = { userId: string; displayName: string; actAt: Date };
}
```

`id` / `createdAt` / `updatedAt`は全モデル共通で自動付与されます。`Model<Job.Data>`が要求する`namespace Job { export type Data = ... }`ブロックは手書き不要で、`disbord generate model` / `disbord migrate` / `disbord model type`実行のたびに`@Column`/`@Relate`のメタデータから機械的に導出してファイル末尾へ自動生成されます（`timestamp_ms`のaccessorだけは読み取り時`Dayjs`・書き込み時`Date`と非対称なため、`Job.Data`側は`Date`のまま、Model内の型定義は`Dayjs`にします。`Job.create()`/`find()`/`update()`等の入力型はこの`Job.Data`を経由するため、キー名のtypoやカラム追加漏れがコンパイルエラーで検知できます）。接続先はTurso用の環境変数の有無で自動判定され（未設定ならローカルsqlite`.disbord/db/dev.db`）、中身は`disbord studio`で確認できます。
